...
function findRootRelativeLocalAssets(html) {
  const assetRefs = [...html.matchAll(/\b(?:src|href)=["']([^"']+)["']/g)].map(
    (match) => match[1],
  );
...
assert(indexHTML.includes('property="og:image:alt"'), "index.html must declare og:image:alt");
assert(indexHTML.includes('name="twitter:card" content="summary_large_image"'), "twitter card must be large");
assert(mainJS.includes('import "./style.css";'), "main.js must import ./style.css");
assert(viteConfig.includes('base: "./"'), "vite.config.js must keep base set to ./");

for (const domID of requiredDOMIDs) {
  assert(indexHTML.includes(`id="${domID}"`), `index.html is missing required id="${domID}"`);
}

for (const tabID of ["homebrew", "source", "env"]) {
  assert(indexHTML.includes(`data-tab="${tabID}"`), `index.html is missing data-tab="${tabID}"`);
  assert(indexHTML.includes(`id="panel-${tabID}"`), `index.html is missing panel-${tabID}`);
}

const rootRelativeAssets = findRootRelativeLocalAssets(indexHTML);
assert(
  rootRelativeAssets.length === 0,
  `index.html contains root-relative local asset paths: ${rootRelativeAssets.join(", ")}`,
);
